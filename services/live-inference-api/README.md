...
```json
{
  ...
  "subject_id": "optional-stable-user-id"
}
```

Optional `subject_id` on any `stream_batch` binds the WebSocket session to a user for personalization lookup when `LIVE_INFERENCE_PERSONALIZATION_BASE_URL` is set. `session_reset` clears it.
...
When personalization base URL is configured, `inference` may include `personalization`: `subject_id`, `profile` (full JSON from `personalization-worker` or `null` if unknown), and optional `error` if the HTTP fetch failed. If the base URL is unset, the field is omitted.

When the profile includes `physiology_baseline`, the service may apply **L1** z-score normalization to features before `predict` and echo `personalization.l1` when applied.

When `adaptation_state.active_personalization_level` is `light` or `full` and `adaptation_state.l2_calibration.output_label_maps` is present, the service applies **L2** post-hoc label mapping on `activity`, `arousal_coarse`, and `valence_coarse` after the global model; optional `l2_calibration.global_model_reference_match` restricts mapping to profiles whose `global_model_reference` matches. Echo `personalization.l2` when L2 changed any label.
...
- `LIVE_INFERENCE_WINDOW_MS` (default: 1000) — window size
- `LIVE_INFERENCE_STEP_MS` (default: 5000) — step between windows
- `LIVE_INFERENCE_PERSONALIZATION_BASE_URL` (optional) — e.g. `http://personalization-worker:8110`; enables per-window profile snapshot on `inference`
- `LIVE_INFERENCE_PERSONALIZATION_TIMEOUT_S` (default: 2) — HTTP timeout for profile GET
...
`POST /v1/replay/infer` (и `POST /api/v1/replay/infer`) дергает `replay-service`, прогоняет те же фичи и глобальный `predict`, что и WebSocket, и возвращает список окон с полями `activity` / `arousal_coarse` / `valence_coarse`. Для batch-доставки L2 в `personalization-worker` после калибровочной сессии см. `scripts/push_l2_from_replay_infer.py`.
