...
3. Для загрузки артефактов в MinIO API подставляет в `upload_url` только базовый URL из `INGEST_S3_PRESIGN_ENDPOINT_URL` (тот же, что для подписи в `POST .../artifacts/presign`). Создать `.env` в корне репозитория (рядом с `docker compose`) или экспортировать переменную перед запуском. Для проверки с реального iPhone без ослабления ATS нужен **HTTPS** на том хосте, который видит телефон (например reverse-proxy с TLS), и тогда `INGEST_S3_PRESIGN_ENDPOINT_URL=https://s3....` — иначе `http://...` даст отказ ATS. Для локальной сети без TLS (только симулятор или исключение ATS) возможен вариант `http://<MAC_IP>:9000`.
